...
export default function Profile() {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 py-10 px-4 sm:px-8 overflow-hidden">
      ...
      <motion.div
        ...
        className="relative z-10 max-w-6xl mx-auto bg-white/80 backdrop-blur-2xl border border-gray-200 shadow-2xl rounded-3xl overflow-hidden"
      >
        {/* 🔷 Banner */}
        <div className="h-36 sm:h-44 bg-gradient-to-r from-indigo-600 to-purple-600 relative">
          <img
            src={user?.photo}
            alt="Profile"
            className="absolute left-6 sm:left-10 -bottom-14 w-28 h-28 sm:w-32 sm:h-32 rounded-full border-4 border-white shadow-xl object-cover"
          />
        </div>

        {/* 👤 Header Info */}
        <div className="px-6 sm:px-10 pt-16 sm:pt-20 pb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
          ...
          {!user ? (
            <motion.button
              ...
              className="bg-gradient-to-r from-red-500 to-orange-500 text-white px-6 py-2.5 rounded-xl font-medium shadow-md hover:shadow-xl transition-all"
            >
              Sign in with Google
            </motion.button>
          ) : (
            <motion.button
              ...
              className="flex items-center gap-2 bg-gray-200 text-gray-800 px-6 py-2.5 rounded-xl font-medium hover:bg-gray-300 transition"
            >
          ...
          {isEditing ? (
            <>
              ...
              <div className="md:col-span-2 flex flex-col sm:flex-row gap-4 mt-4">
                <motion.button whileHover={{ scale: 1.05 }} onClick={handleSave} className="flex-1 bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-3 rounded-xl font-semibold shadow hover:shadow-xl">
                  Save Profile
                </motion.button>
                <motion.button whileHover={{ scale: 1.05 }} onClick={toggleCompanies} className="flex-1 bg-indigo-100 text-indigo-700 py-3 rounded-xl font-semibold hover:bg-indigo-200">
            ...
            <>
              ...
              <div className="md:col-span-2 flex flex-col sm:flex-row gap-4 mt-4">
                <motion.button whileHover={{ scale: 1.05 }} onClick={handleEdit} className="flex-1 bg-gray-200 text-gray-800 py-3 rounded-xl font-semibold hover:bg-gray-300">
                  Edit Profile
                </motion.button>
                <motion.button whileHover={{ scale: 1.05 }} onClick={toggleCompanies} className="flex-1 bg-indigo-100 text-indigo-700 py-3 rounded-xl font-semibold hover:bg-indigo-200">
                  {showCompanies ? "Hide Applied Jobs" : "View Applied Jobs"}
        ...
        {/* 🏢 Applied Jobs */}
        {showCompanies && (
          <div className="p-8 border-t border-gray-200 bg-white/70 backdrop-blur-lg">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {applications.map((app, i) => (
                  <motion.div
                    ...
                    className="bg-white/80 border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-xl transition"
                  >
...
function InfoCard({ title, content, icon }) {
  return (
    <motion.div
      ...
      className="bg-white/80 backdrop-blur-lg border border-gray-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition"
    >
      <div className="flex items-center gap-2 mb-2 text-indigo-600 font-semibold">
        {icon}
        <h4>{title}</h4>
      </div>
      <p className="text-gray-700 text-sm">{content || "No information added yet."}</p>
    </motion.div>
  );
}
